...
static void init(QTextBoundaryFinder::BoundaryType type, const ushort* chars, int length, HB_CharAttributes *attributes)
{
	HB_ScriptItem *scriptItems = new HB_ScriptItem[length];
	int count = 0;
    ...
    while (uc < e) {
        ...
        if (script != lastScript) {
            if (uc != start) {
                scriptItems[count].pos = start - string;
                scriptItems[count].length = uc - start;
                scriptItems[count].script = (HB_Script)lastScript;
                start = uc;
                lastScript = script;
				count++;
            }
    ...
    qGetCharAttributes(string, length, scriptItems, count, attributes);
    if (type == QTextBoundaryFinder::Word)
        HB_GetWordBoundaries(string, length, scriptItems, count, attributes);
    else if (type == QTextBoundaryFinder::Sentence)
        HB_GetSentenceBoundaries(string, length, scriptItems, count, attributes);
	if (scriptItems)
		delete []scriptItems;
}
...
QTextBoundaryFinder::QTextBoundaryFinder(const QTextBoundaryFinder &other)
    ...
    , freePrivate(true)
{
    d = (QTextBoundaryFinderPrivate *) qMalloc(length*sizeof(HB_CharAttributes));
...
QTextBoundaryFinder &QTextBoundaryFinder::operator=(const QTextBoundaryFinder &other)
{
    ...
    d = (QTextBoundaryFinderPrivate *) qRealloc(d, length*sizeof(HB_CharAttributes));
    memcpy(d, other.d, length*sizeof(HB_CharAttributes));
...
/*!
  ...
  \a buffer.
*/
QTextBoundaryFinder::QTextBoundaryFinder(BoundaryType type, const ushort* chars, int len, unsigned char *buffer, int bufferSize)
    : t(type)
    , chars(chars)
    , length(len)
    , pos(0)
