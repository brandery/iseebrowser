...
void FrameLoaderClient::dispatchDecidePolicyForNewWindowAction(FramePolicyFunction policyFunction, const NavigationAction& action, const ResourceRequest& request, PassRefPtr<FormState> , const String& frameName)
{
	ASSERT(frame() && policyFunction);
...
void FrameLoaderClient::dispatchDecidePolicyForNavigationAction(FramePolicyFunction policyFunction, const NavigationAction& action, const ResourceRequest& request, PassRefPtr<FormState> formState)
{
	// FIXME
    ASSERT(frame() && policyFunction);
    if (!frame() || !policyFunction)
...
void FrameLoaderClient::dispatchDidFailLoading(DocumentLoader* docLoader, unsigned long  identifier, const ResourceError& error)
{
	...
	{
		const ResourceRequest& request = docLoader->request();
