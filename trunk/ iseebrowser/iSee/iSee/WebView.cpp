...
void WebView::didFailLoadingWithError(unsigned long identifier, const ResourceError& error)
{
	...
	if (frameLoader)
	{
		...
		if (errorPage)
		{
			...
			frameLoader->load(ResourceRequest(failUrl), data);
...
{
	load(ResourceRequest(autocorrectURL(url)));
}

void WebView::load(const ResourceRequest &request)
{
...
// 创建一个新窗口
WebView* WebView::createWebViewWithRequest(bool evented, const ResourceRequest& request)
{
...
{
	return newWebView(ResourceRequest(autocorrectURL(String(url))), String(frameName), String(groupName));
}

WebView *WebView::newWebView(const ResourceRequest &request, const String &frameName, const String &groupName)
{
